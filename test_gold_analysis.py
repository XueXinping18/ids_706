...
class TestFiltering:
    ...
    def test_filter_and_group_invalid_dates(self):
        ...
        df = pd.DataFrame(
            {"SPX": [100, 200, 300], "Date": ["invalid", "date", "values"]}
        )
        result = enrich_with_year_and_flags(df)
        assert isinstance(result, pd.DataFrame)
...
class TestMachineLearning:
    ...
    def test_ml_analysis_sufficient_data(self):
        ...
        models = train_and_evaluate_models(df)
        assert len(models) == 6
    ...
    def test_ml_analysis_single_column(self):
        ...
        df = pd.DataFrame({"SPX": [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]})
        models = train_and_evaluate_models(df)
...
class TestVisualization:
    ...
    def test_create_plots_with_data(self):
        ...
        df = pd.DataFrame({"SPX": [100, 200, 300], "year": [2020, 2021, 2022]})
